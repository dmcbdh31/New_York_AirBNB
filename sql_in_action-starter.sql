...
SELECT COUNT(id) as number_of_rows_in_table FROM final_airbnb; 
...
SELECT COUNT(DISTINCT neighbourhood) as neighbourhoods_listed FROM final_airbnb;
...
SELECT MAX(price) as hightest_price FROM final_airbnb;

SELECT MIN(price) as lowest_price FROM final_airbnb;
...
SELECT COUNT(number_of_reviews) as listings_with_no_review FROM final_airbnb Where number_of_reviews = "no value";
...
SELECT neighbourhood FROM final_airbnb GROUP BY neighbourhood ORDER BY COUNT(neighbourhood) DESC LIMIT 1;
